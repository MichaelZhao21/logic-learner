...
namespace Wireform.Sketch
{
    public class WireformSketch : IDisposable
    {
        ...
        private OneOf<string, DocumentData> FindDocument(Mat frame)
        {
            ...
            var f_transformation = CvInvoke.GetPerspectiveTransform(f_initialDoc, f_transformedDoc);
            f_transformationG = f_transformation.Clone();
        ...
        private string FindGates(DocumentData doc)
        {
            ...
            CvInvoke.GaussianBlur(d_blurred, d_blurred, new Size(3, 3), 0);
        ...
        private void DrawOutput(Mat frame, DocumentData doc)
        {
            ...
            if (Props.DrawOutput)
            {
                ...
                foreach(BitSource gate in boardStack.CurrentState.Gates.Where(x => x is BitSource).Cast<BitSource>())
                {
                    if(gate.Value == BitValue.One)
                    {
                        CvInvoke.Circle(doc.Document, gate.StartPoint.ToPoint(), 11, new MCvScalar(), thickness: -1);
                        CvInvoke.Circle(doc.Document, gate.StartPoint.ToPoint(), 8, Props.BitColors.One, thickness: -1);
                    }
                    else if(gate.Value == BitValue.Zero)
                    {
                        CvInvoke.Circle(doc.Document, gate.StartPoint.ToPoint(), 11, new MCvScalar(), thickness: -1);
                        CvInvoke.Circle(doc.Document, gate.StartPoint.ToPoint(), 8, Props.BitColors.Zero, thickness: -1);
                    }
                }
            }
